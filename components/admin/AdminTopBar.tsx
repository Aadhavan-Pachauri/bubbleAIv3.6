...
import { Bars3Icon, ChevronLeftIcon, ChevronDownIcon } from '@heroicons/react/24/outline';
      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        <div className="hidden lg:flex items-center gap-2 p-1 bg-black/20 rounded-lg">
            {activeProject ? (
                <div className="flex items-center gap-4 px-2">
                    ...
                    <span className="text-sm font-semibold text-white">{activeProject.name}</span>
                </div>
            ) : (
                navItems.map(item => (
                    <button
        ...
        <div className="relative lg:hidden" ref={navRef}>
            {activeProject ? (
                 <button
                    onClick={onSwitchToCocreator}
                    className="flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg bg-black/20 text-white hover:bg-black/40"
                >
                    <ChevronLeftIcon className="w-4 h-4" />
                    <span>Back</span>
                </button>
