...
export const useChat = ({ user, geminiApiKey, workspaceMode, adminProject }: UseChatProps) => {
    ...
    // Track previous chat ID to prevent unnecessary message clearing
    ...
    useEffect(() => {
        ...
        // Only clear messages if we have actually switched to a DIFFERENT chat ID
        // AND the previous ID wasn't null (initial load)
    ...
    useEffect(() => {
        ...
        const fetchChats = async () => {
            // Only show global loading on first mount/fetch
    ...
    useEffect(() => {
        ...
        const fetchMessages = async () => {
            if (activeChat) {
                ...
                // Don't show loading if we are just sending a message (optimistic UI handles it)
                ...
                try {
                    ...
                    if (!isCancelled && isMountedRef.current && activeChatIdRef.current === chatId) {
                        setMessages(prev => {
                            // CRITICAL FIX: Merge history with pending optimistic messages
    ...
    const handleSendMessage = useCallback(async (
      ...
      try {
        ...
        // Ensure activeChatIdRef is up to date for this new operation
        ...
        // --- TITLE GENERATION ---
        // Relaxed check: Just generate if short history
        if (text.trim()) {
        ...
        const onStreamChunk = (chunk: string) => {
            ...
            // Allow if on current chat OR if active ID is not set yet (navigation race condition)
